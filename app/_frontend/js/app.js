...
const termData = {
  ...
  'process-log': '<strong>Process Log</strong> reports the current request state and the final observed VoxCPM2 generation speed. The <code>it/s</code> value is the final smoothed tqdm rate, so it can be lower than a faster number shown earlier in the terminal. Inference Timesteps are the main synthesis-setting speed tradeoff; GPU clocks, temperature, competing GPU work, and the generated sequence can also move the result.',
  ...
  options: '<strong>Options</strong> controls optional local features. Enable Clone / Continue before those consent-sensitive tools appear, enable Local LLM Feedback before provider fields are shown, toggle VRAM warnings or telemetry visibility, and use <em>1.5x Text Size of Spoken Script / Voice Design</em> to enlarge only those two text areas.',
